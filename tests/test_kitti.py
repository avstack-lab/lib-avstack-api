...
def test_get_image():
    if DM_obj is not None:
        ...
        assert img.frame == 1
        assert img.shape == (DM_obj.CFG['IMAGE_HEIGHT'], DM_obj.CFG['IMAGE_WIDTH'], DM_obj.CFG['IMAGE_CHANNEL'])
    else:
...
def test_convert_krd():
    if DM_raw is not None:
        dates = DM_raw.get_available_dates()
        exp_path = DM_raw.convert_sequence(dates[0], idx_seq=0,
                                        max_frames=None, max_time=None, tracklets_req=True, path_append='-experiment')
