...
from .._nu_base import _nuBaseDataset
...
class nuImagesDataset(_nuBaseDataset):
    ...
    sensors = {'main_camera':'CAM_FRONT'}
    keyframerate = 2
    sensor_IDs = {'CAM_BACK':3, 'CAM_BACK_LEFT':1, 'CAM_BACK_RIGHT':2,
                  'CAM_FRONT':0, 'CAM_FRONT_LEFT':4, 'CAM_FRONT_RIGHT':5}

    def __init__(self, data_dir, split, verbose=False, whitelist_types=..., ignore_types=...):
        nuim = NuImages(version=split, dataroot=data_dir, verbose=verbose)
        self.t0 = 0
        self.ego_speed_interp = None
        super().__init__(nuim, None, data_dir, split, verbose, whitelist_types, ignore_types)

    def sensor_name(self, frame):
        return self.nuX.get('sensor', self._get_calib_data(frame, None)['sensor_token'])['channel']
