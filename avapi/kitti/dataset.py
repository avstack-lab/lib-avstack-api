...
from avstack.geometry import bbox, Origin, NominalOriginStandard, q_mult_vec
from avstack.geometry import Translation, Rotation, Transform, q_stan_to_cam
...
class KittiObjectDataset(BaseSceneDataset):
    ...
    framerate = 10
    folder_names  = {'image-2':'image_2', 'lidar':'velodyne', 'label':'label_2',
                            'calibration':'calib', 'ground':'planes', 'disparity':'disparity',
                            'timestamps':'timestamps'}
    data_endings = {'image-2':'png', 'lidar':'bin', 'label':'txt', 'calibration':'txt'}
    ...
    def __init__(self, data_dir, split, whitelist_types=_nominal_whitelist_types,
    ...
    def _load_ego(self, frame):
        ...
        h, w, l = self.ego_size
        box = bbox.Box3D([h, w, l, np.zeros((3,)), np.quaternion(1)], NominalOriginStandard)
        ego.set(self.get_timestamp(frame), pos, box)
        return ego

    def check_frame(self, frame):
        assert frame in self.frames, f'Candidate frame, {frame}, not in frame set {self.frames}'
    ...
    def _load_calibration(self, frame, sensor):
        calib_fname = os.path.join(self.split_path, self.folder_names['calibration'], '%06d.txt'%frame)
    ...
    def _load_calibration_from_file(file, sensor, img_shape, ego_height=1.49):
        ...
        if 'image' in sensor.lower():
            ...
            P = np.reshape(calib_data['P%s' % sensor.lower().replace('image-', '')], [3,4])
            ...
            q_C0_2_L = tforms.transform_orientation(R_velo_to_cam0, 'dcm', 'quat').conjugate()
    ...
    def _load_image(self, frame, sensor):
        if isinstance(sensor, str) and 'image' in sensor:
            img_fname = os.path.join(self.split_path, self.folder_names[sensor], '%06d.png'%frame)
        else:
            img_fname = os.path.join(self.split_path, self.folder_names['image-%i'%sensor], '%06d.png'%frame)
        return imread(img_fname)[:,:,::-1]

    def _load_lidar(self, frame, sensor, **kwargs):
        filter_front=True  # always filter KITTI to front-only
        lidar_fname = os.path.join(self.split_path, self.folder_names['lidar'], '%06d.bin'%frame)
    ...
    def _load_objects(self, frame, sensor, whitelist_types=['Car', 'Pedestrian', 'Cyclist'], ignore_types=['DontCare']):
        # assert sensor in [None, 'image-2']
        label_fname = os.path.join(self.split_path, self.folder_names['label'], '%06d.txt'%frame)
        object_calib = self.get_calibration(frame, 'image-2')
        objects = self._load_objects_from_file(label_fname, whitelist_types, ignore_types)
        return objects

    def _load_timestamp(self, frame, sensor='lidar'):
        if os.path.exists(os.path.join(self.split_path, self.folder_names['timestamps'])):
            ts_fname = os.path.join(self.split_path, self.folder_names['timestamps'], '%06d.txt'%frame)
    ...
    def _save_calibration(self, frame, calib, folder, sensor_ID=2):
        calib_text_str = 'P{}: {}'.format(sensor_ID, ' '.join([str(p) for p in np.reshape(calib.P, -1)]))
    ...
    def _get_sensor_file_name(self, frame, sensor: str):
        return os.path.join(self.split_path, self.folder_names[sensor], '%06d.%s' % (frame, self.data_endings[sensor]))
...
class KittiScenesManager(BaseSceneManager):
    ...
    def __init__(self, data_dir, raw_data_dir=None, convert_raw=False):
        ...
        # Get scenes
        self.scenes = [glob.glob(os.path.join(self.data_dir, date + '*')) for date in self.date_names]
        self.scenes = sorted([scene.replace(data_dir,'').lstrip('/') for scenes_this in self.scenes for scene in scenes_this])
    ...
    def convert_raw_dataset(self):
        if self.raw_data_dir is not None:
            ...
            for idx_date, idx_seq in tqdm(self.scene_tuples):
                ...
                exp_path = KRD.convert_sequence(self.date_names[idx_date], idx_seq=idx_seq, max_frames=None, max_time=None, tracklets_req=True)
        else:
...
class KittiRawDataset():
    ...
    def convert_sequence(self, date, id_seq=None, idx_seq=None, iframe_start=0, max_frames=None, max_time=None, tracklets_req=True, path_append='', verbose=True):
        """
        :date - the date (as string) of the sequence capture
        ...
        # -- calibration
        c2c = KittiObjectDataset.read_dict_text_file(os.path.join(self.data_dir, date, 'calib_cam_to_cam.txt'))
        v2c = KittiObjectDataset.read_dict_text_file(os.path.join(self.data_dir, date, 'calib_velo_to_cam.txt'))
        i2v = KittiObjectDataset.read_dict_text_file(os.path.join(self.data_dir, date, 'calib_imu_to_velo.txt'))
        calibs = {}
        ...
        tracklets = xmlParser.parseXML(os.path.join(seq_folder, 'tracklet_labels.xml'), verbose=verbose)
        for itrk, trk in enumerate(tracklets):
            ...
            for trans, rot, state, occ, trunc, amtOcclusion, amtBorders, iframe in trk:
                ...
                # Store fields
                box3d = bbox.Box3D([h, w, l, x_C2_2_obj_in_C2, q_C2_2_obj], origin=obj_origin)
                ...
                att = box3d.q
                ang = None
                obj.set(ts, pos, box3d, vel,
                    acc, att, ang, origin=obj_origin)
