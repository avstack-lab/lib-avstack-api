...
class CarlaSceneDataset(BaseSceneDataset):
    ...
    def __init__(self, data_dir, scene, whitelist_types=_nominal_whitelist_types,
        ...
        for filename in sorted(glob.glob(os.path.join(self.obj_folder, '*.txt'))):
            ...
            ts = float(ts.split('_')[1]); frame = int(frame.split('.')[0].split('_')[1])
            if 'ego' in filename:
        ...
        if os.path.exists(sensor_data_folder):
            ...
            for sens in sorted(next(os.walk(self.sensor_data_folder))[1]):
                ...
                for i, filename in enumerate(sorted(glob.glob(os.path.join(sensor_folders[name], 'data-*')))):
                    filename = filename.split('/')[-1]
                    if i == 0:
    ...
    def get_sensor_file(self, frame, timestamp, sensor, file_type):
        ...
        filepath = os.path.join(self.sensor_folders[sensor], file_type + '-' + file_post)
        return filepath

    def _load_sensor_data_filepath(self, frame, sensor):
        return self.get_sensor_file(frame, None, sensor, 'data') + self.file_endings[sensor]
    ...
    def _load_lidar(self, frame, sensor, filter_front, with_panoptic=False):
        ...
        if filepath.endswith('.ply'):
            ...
            pcd = np.fromfile(filepath, dtype=np.float32).reshape((-1, self.CFG['num_lidar_features']))
        if filter_front:
    ...
    def _load_objects(self, frame, sensor, whitelist_types=['car', 'truck', 'bicycle', 'motorcycle'], ignore_types=[]):
        if sensor is None:
            sensor = 'CAM_FRONT'
        timestamp = None
        filepath = self.get_object_file(frame, timestamp, sensor=sensor, is_ego=False, is_global=False)
        objs = self._read_objects(filepath)
        return np.array([obj for obj in objs if ((obj.obj_type in whitelist_types) or (whitelist_types=='all')) and (obj.obj_type not in ignore_types)])
