...
import os, sys, datetime, math, weakref
import carla
...
from avstack.geometry import bbox, Translation, Rotation, Origin
...
class CameraDisplayManager(object):
    def __init__(self, parent, pg_display, hud, gamma_correction):
        ...
        self.representation_colors = {'object_3d':(0,0,255), 'object_2d':(0,255,0), 'track_3d':(255,255,255)}
        self.idx_representation = 2-1
        self.toggle_obj_representation()
        self.R_UE2C = np.array([[ 0,  1,  0 ],
                                [ 0,  0, -1 ],
                                [ 1,  0,  0 ]]).T
    ...
    def add_camera(self, name, P, tform, attachment):
        ...
        self._camera_bbox[name].append(bbox.Box2D([0,0,img_shape[0], img_shape[1]], cam_calib))
    ...
    def add_hud_camera(self):
        ...
        camera_P = np.array([[f, 0, w/2.0, 0],
                             [0, f, h/2.0, 0],
                             [0, 0,   1  , 0]])

        self.add_camera('hud', camera_P, carla.Transform(carla.Location(x=-8.5, z=3.5)), carla.AttachmentType.Rigid)
        self.add_camera('hud', camera_P, carla.Transform(carla.Location(z=15), carla.Rotation(pitch=-90)), carla.AttachmentType.Rigid)
    ...
    def toggle_view(self):
        self.transform_index = (self.transform_index + 1) % len(self._camera_transforms[self.sensor_view])
        self.set_sensor(self.index, name=self.sensor_view, notify=False, force_respawn=True)
    ...
    def set_sensor(self, index, name='hud', notify=True, force_respawn=False):
        ...
        if needs_respawn:
            ...
            self.sensor.listen(lambda image: CameraDisplayManager._parse_image(weak_self, image))
        if notify:
    ...
    def toggle_obj_representation(self):
        self.idx_representation = (self.idx_representation+1) % len(self.object_representations)
        print(f'Representation set as: {self.object_representations[self.idx_representation]}')
...
class HUD(object):
    ...
    def tick(self, world, ego, clock):
        ...
        self._info_text = [
            ...
            '',
            'Speed:   % 15.0f m/s' % (math.sqrt(v_truth.x**2 + v_truth.y**2 + v_truth.z**2)),
            'Location (avstack):% 20s' % ('(% 5.1f, % 5.1f)' % (t_truth.location.x, -t_truth.location.y)),
        ...
        if len(vehicles) > 1:
            ...
            distance = lambda l: math.sqrt((l.x - t_truth.location.x)**2 + (l.y - t_truth.location.y)**2 + (l.z - t_truth.location.z)**2)
            vehicles = [(distance(x.get_location()), x) for x in vehicles if x.id != ego.actor.id]
