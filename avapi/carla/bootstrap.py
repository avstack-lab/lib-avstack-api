...
from . import sensors, config, utils
...
def apply_world_settings(world, cfg):
    ...
    for i in range(n_walkers):
        spawn_points.append(carla.Transform(world.get_random_location_from_navigation(), carla.Rotation()))
        ...
        for i in range(n_agents):
            ...
            if npc is not None:
                if 'walker' in npc.type_id:
                    ai_controller = world.try_spawn_actor(walker_controller_bp, carla.Transform(), npc)
                    ai_controller.start()
...
def bootstrap_display(world, ego, cfg=None, config_file='./default_display.yml'):
    ...
    if cfg['enabled']:
        ...
        pg_display = pygame.display.set_mode(
            (cfg['disp_x'], cfg['disp_y']),
            pygame.HWSURFACE | pygame.DOUBLEBUF)

        # -- display manager
        hud = display.HUD(cfg['disp_x'], cfg['disp_y'])
        world.on_tick(hud.on_world_tick)
        display_manager = display.CameraDisplayManager(ego, pg_display, hud, gamma_correction=2.2)
        try:
...
def bootstrap_ego(world, ego_stack, cfg=None, config_file='./default_ego.yml', save_folder=''):
    if cfg is None:
        cfg = config.read_config(config_file)
...
def bootstrap_infrastructure(world, cfg, config_file='./default_infrastructure.yml', save_folder=''):
    if cfg is None:
        cfg = config.read_config(config_file)
...
def bootstrap_infra_sensor(infra, idx, cfg, save_folder):
    ...
    if cfg['idx_spawn'] == 'randint':
        ...
        spawn_point = spawn_points[cfg['idx_spawn']] if cfg['idx_spawn'] else random.choice(spawn_points)
    x_spawn = utils.carla_location_to_numpy_vector(spawn_point.location) + \
        utils.carla_location_to_numpy_vector(carla.Location(x=cfg['transform']['location']['x'],
    ...
    q_spawn = utils.carla_rotation_to_quaternion(
        carla.Rotation(pitch=cfg['transform']['rotation']['pitch'],
                       yaw=cfg['transform']['rotation']['yaw'],
                       roll=cfg['transform']['rotation']['roll'])) * \
        utils.carla_rotation_to_quaternion(spawn_point.rotation)

    tform_spawn = carla.Transform(utils.numpy_vector_to_carla_location(x_spawn),
    ...
    sens = sens(source_name, infra, tform_spawn, cfg['attributes'], cfg['mode'],
        cfg['noise'], save=cfg['save'], save_folder=save_folder)

    # -- add to infra
    infra.add_sensor(source_name, sens, comm_range=cfg['comm_range'], pos_covar=pos_covar)
...
def bootstrap_ego_sensor(ego, ID, cfg, save_folder):
    ...
    save_folder = os.path.join(save_folder, 'sensor_data')
    tform = carla.Transform(carla.Location(cfg[k1]['transform']['location']['x'],
                                           cfg[k1]['transform']['location']['y'],
                                           cfg[k1]['transform']['location']['z']),
                            carla.Rotation(cfg[k1]['transform']['rotation']['pitch'],
    ...
    ego.add_sensor(k1, sens(source_name, ego, tform, cfg[k1]['attributes'], cfg[k1]['mode'],
        cfg[k1]['noise'], save=cfg[k1]['save'], save_folder=save_folder))
...
def bootstrap_npcs(world, cfg, verbose=False):
    ...
    for item in cfg:
        ...
        # -- spawn
        spawn_points = world.get_map().get_spawn_points()
        spawn_point = spawn_points[cfg[item]['idx_spawn']] if cfg[item]['idx_spawn'] else random.choice(spawn_points)
...
def bootstrap_standard(world, traffic_manager, ego_stack, cfg, save_folder):
    ...
    try:
        ...
        raise e
    try:
        infra = bootstrap_infrastructure(world, cfg['infrastructure'], save_folder=save_folder)
    except Exception as e:
        ego.destroy()
        for npc in npcs_set:
            npc.destroy()
    ...
    manager = CarlaManager(world, traffic_manager,
        record_truth=cfg['recorder']['record_truth'], record_folder=save_folder)
    manager.ego = ego
