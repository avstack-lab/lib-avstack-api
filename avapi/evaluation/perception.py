...
from avstack.modules.perception.detections import get_detections_from_file, BoxDetection
from avapi.evaluation.base import ResultManager, ResultAnalyzer
# from avapi.evaluation.base import ResultManager


def get_percep_results_from_folder(DM, result_path, idxs=None, sensor_eval=None,
    ...
    PRA = PercepResultsAnalyzer(DM, result_path=result_path, idxs=idxs,
...
def get_percep_results_from_multi_folder(DM, result_base, *args, **kwargs):
    ...
    for subdir in next(os.walk(result_base))[1]:
        ...
        res_all[subdir] = get_percep_results_from_folder(DM, result_path, *args, **kwargs)
    return res_all
...
class PercepResultsAnalyzer(ResultAnalyzer):
    ...
    def _run_per_frame_analysis(DM, result_path, sensor_eval, sensor_eval_super,
        ...
        metric = '3D_IoU' if (len(dets)==0) or (isinstance(dets[0].box, bbox.Box3D)) else '2D_IoU'
        result = ResultManager(idx, dets, truths_whitelist, truths_dontcare, metric=metric)
        ...
        metrics['precision'] = metrics['nTP']/(metrics['nTP']+metrics['nFP']) if (metrics['nTP'] + metrics['nFP'])>0 else 0
    ...
    def _run_per_seq_analysis(self, per_frame_metrics, *args, **kwargs):
        ...
        return {'tot_TP':sum(tp_per_frame),
