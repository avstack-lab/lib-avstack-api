...
from avapi.evaluation.base import ResultManager, ResultAnalyzer
from trackeval.datasets._base_dataset import _BaseDataset as _TrkEvalBaseDataset
from trackeval.metrics import HOTA, CLEAR, TrackMAP, VACE, IDEucl
from trackeval.utils import init_config as init_track_dataset_config
from trackeval.utils import get_code_path
from trackeval import _timing, Evaluator as TrackEvaluator


def get_track_results_from_folder(DM, result_path, idxs=None, sensor_eval=None,
        sensor_eval_super=None, max_dist=None, max_occ=None, whitelist_types=None,
        run_tqdm=True, multiprocess=True):
    if whitelist_types is None:
        whitelist_types = DM.nominal_whitelist_types
    TRA = TrackResultsAnalyzer(DM, result_path=result_path, idxs=idxs,
        sensor_eval=sensor_eval, sensor_eval_super=sensor_eval_super,
        max_dist=max_dist, max_occ=max_occ, whitelist_types=whitelist_types,
        run_tqdm=run_tqdm, multiprocess=multiprocess)
    return TRA.results()
...
def get_track_results_from_multi_folder(DM, result_base, *args, **kwargs):
    ...
    for subdir in next(os.walk(result_base))[1]:
        ...
        res_all[subdir] = get_track_results_from_folder(DM, result_path, *args, **kwargs)
    return res_all


class TrackResultsAnalyzer(ResultAnalyzer):
    def results(self):
        res_frame, res_seq = self._results_from_folder()
        return res_frame, res_seq

    @staticmethod
    def _run_per_frame_analysis(DM, result_path, sensor_eval, sensor_eval_super,
            max_dist, max_occ, whitelist_types, idx):
        """Load track status of a single frame"""
        # Get truths
        dist_all = max_dist if sensor_eval_super == 'ego' else None
        truths_all = DM.get_objects(idx, sensor=sensor_eval_super, max_dist=dist_all, whitelist_types='all')
        truths_whitelist = DM.get_objects(idx, sensor=sensor_eval, max_dist=max_dist,
            max_occ=max_occ, whitelist_types=whitelist_types)
        whitelist_ids = [truth.ID for truth in truths_whitelist]
        truths_dontcare = np.asarray([tru for tru in truths_all if tru.ID not in whitelist_ids])

        # Get tracks
        trk_file_path = os.path.join(result_path, '%06i.txt' % idx)
        tracks = DM.get_objects_from_file(trk_file_path, whitelist_types=whitelist_types)

        # -- 2d analysis

        # -- 3d analysis
        result_3d = ResultManager(idx, tracks, truths_whitelist, truths_dontcare, metric='3D_IoU')

        # -- package up
        metrics = {'result':result_3d,
    ...
    def _run_per_seq_analysis(self, per_frame_metrics, *args, **kwargs):
        ...
        return {'tot_TT':sum(tt_per_frame),
...
class AvstackTrackDataset(_TrkEvalBaseDataset):
    ...
    def __init__(self, SD, trk_folder, tracker_name, sensor_eval=None,
            sensor_eval_super=None, max_dist=None, classes='nominal',
            whitelist_types=None, ignore_types=None, output_folder='metrics',
            output_subfolder='tracking', track_has_class=False):
        """Folders using avstack convention"""
        self.SD = SD
        self.trk_folder = trk_folder
        self.tracker_list = [tracker_name]
        self.sensor_eval = sensor_eval
        self.sensor_eval_super = sensor_eval_super if sensor_eval_super is not None else sensor_eval
        ...
        self.config = init_track_dataset_config(None, self.get_default_dataset_config(), self.get_name())
    ...
    def _calculate_similarities(self, gt_tracks, trk_tracks, cost_thresh=0.02, metric='center_dist', radius=4):
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        if is_gt:
            ...
            for idx in self._get_indices_available():
                ...
                truths_all = self.SD.get_objects(idx, sensor=self.sensor_eval_super, max_dist=dist_all, whitelist_types='all')
                truths_whitelist = self.SD.get_objects(idx, sensor=self.sensor_eval, max_dist=self.max_dist,
                    whitelist_types=self.valid_classes, ignore_types=self.ignore_list)
                whitelist_ids = [truth.ID for truth in truths_whitelist]
                truths_dontcare = np.asarray([tru for tru in truths_all if tru.ID not in whitelist_ids])

                # -- store results
                raw_data['gt_ids'].append(np.array([truth.ID for truth in truths_whitelist]))
                if self.track_has_class:
                    raw_data['gt_classes'].append(np.array([truth.obj_type for truth in truths_whitelist]))
                else:
                    raw_data['gt_classes'].append(np.array(['all_objects' for truth in truths_whitelist]))
            ...
            for idx in self._get_indices_available():
                ...
                tracks = self.SD.get_objects_from_file(trk_file_path,
                    whitelist_types=self.valid_classes, max_dist=self.max_dist)
                # -- store results
                raw_data['tracker_ids'].append(np.array([trk.ID for trk in tracks]))
                if self.track_has_class:
                    raw_data['tracker_classes'].append(np.array([trk.obj_type for trk in tracks]))
                else:
                    raw_data['tracker_classes'].append(np.array(['all_objects' for trk in tracks]))
    ...
    def get_preprocessed_seq_data(self, raw_data, obj_type):
        ...
        """
        data = {'gt_ids':[], 'tracker_ids':[], 'gt_dets':[], 'tracker_dets':[], 'similarity_scores':[]}
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            sim_scores = raw_data['similarity_scores'][t][gt_class_mask,:][:,trk_class_mask]

            # 2) perform assignment, remove anything near "ignores"
            ignore_cols = []
            if len(raw_data['gt_ignores'][t]) > 0:
                sim_scores_ignores = self._calculate_similarities(raw_data['gt_ignores'][t], trk_trks, metric='center_dist')
                assign_sol = gnn_single_frame_assign(-sim_scores_ignores, algorithm='JVC',
                    cost_threshold=-0.02, all_assigned=False)
                for r, c in assign_sol.assignment_tuples:
                    ignore_cols.append(c)  # column is the track
            # sanity check:
            if (len(ignore_cols) > 0) and (len(ignore_cols) > np.sum(sim_scores_ignores > 0)):
        ...
        unique_gt_ids = np.array(list({gt_id for gts_id in data['gt_ids'] for gt_id in gts_id}))
        unique_trk_ids = np.array(list({trk_id for trks_id in data['tracker_ids'] for trk_id in trks_id}))
        ...
        for t in range(raw_data['num_timesteps']):
            data['gt_ids'][t] = np.array([gt_id_map[gt_id] for gt_id in data['gt_ids'][t]], dtype=np.int)
            data['tracker_ids'][t] = np.array([trk_id_map[trk_id] for trk_id in data['tracker_ids'][t]], dtype=np.int)
