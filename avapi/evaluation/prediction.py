...
def get_predict_results_from_folder(DM, result_path, idxs=None, sensor_eval=None,
    ...
    PRA = PredictResultsAnalyzer(DM, result_path=result_path, idxs=idxs,
...
def get_predict_results_from_multi_folder(DM, result_base, *args, **kwargs):
    ...
    for subdir in next(os.walk(result_base))[1]:
        ...
        res_all[subdir] = get_predict_results_from_folder(DM, result_path, *args, **kwargs)
    return res_all
...
class PredictResultsAnalyzer(ResultAnalyzer):
    ...
    def _run_per_frame_analysis(DM, result_path, sensor_eval, sensor_eval_super,
            max_dist, max_occ, whitelist_types, idx, max_objects=30):
        ...
        predictions_raw = DM.get_objects_from_file(pred_file_path, whitelist_types=whitelist_types)
        ...
        for pred in predictions_raw:
            ...
            if pred.t > (ts_available[-1] + 1e-6):
                continue
            predictions[pred.ID][pred.t] = {'prediction':pred, 'truth':None, 'truth_ID':None, 'displacement':None}
        ...
        for frame in frames_all:
            truths_all[frame] = DM.get_objects(frame, sensor=sensor_eval_super,
                max_dist=dist_all, whitelist_types='all')
        ...
        for pred_ID in predictions:
            ...
            if len(asgn_tups) == 1:
                ...
                for t_pred in predictions[pred_ID]:
                    ...
                    frame = time_to_frame_map[t_pred]
                    truths = truths_all[frame]
                    truth_this = [truth for truth in truths if truth.ID==truth_ID_assign]
                    if len(truth_this) == 0:
                        continue
                    elif len(truth_this) == 1:
                        predictions[pred_ID][t_pred]['truth'] = truth_this[0]
                        predictions[pred_ID][t_pred]['truth_ID'] = truth_ID_assign
                        predictions[pred_ID][t_pred]['displacement'] = \
                            predictions[pred_ID][t_pred]['prediction'].position.distance(
    ...
    def _run_per_seq_analysis(self, per_frame_metrics, *args, **kwargs):
        ...
        per-frame runs
        """
        n_with_truth = sum([m['n_with_truth'] for m in per_frame_metrics.values() if m['n_with_truth'] > 0])
        n_tot = sum([m['n_objects'] for m in per_frame_metrics.values() if m['n_objects'] > 0])
        weighted_ADE = [m['ADE']*m['n_with_truth'] for m in per_frame_metrics.values() if m['ADE'] is not None]
        weighted_FDE = [m['FDE']*m['n_with_truth'] for m in per_frame_metrics.values() if m['FDE'] is not None]

        return {'agg_ADE': sum(weighted_ADE)/n_with_truth if n_with_truth>0 else None,
                'agg_FDE': sum(weighted_FDE)/n_with_truth if n_with_truth>0 else None,
                'std_ADE': np.std([m['ADE'] for m in per_frame_metrics.values() if m['ADE'] is not None]),
                'std_FDE': np.std([m['FDE'] for m in per_frame_metrics.values() if m['FDE'] is not None]),
...
def calculate_prediction_metrics(predictions):
    ...
    """
    return {'ADE':calculate_ADE(predictions),
...
def calculate_ADE(predictions):
    ...
    """
    disp_errors = [pred['displacement'] for preds in predictions.values()
                    for pred in preds.values() if pred['displacement'] is not None]
...
def calculate_FDE(predictions):
    ...
    """
    final_disps = []
    final_disps = [[pred['displacement'] for pred in preds.values() if pred['displacement'] is not None]
