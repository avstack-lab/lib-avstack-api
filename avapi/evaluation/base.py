...
from avstack.utils.maskfilters import box_in_fov
...
class ResultAnalyzer():
    def __init__(self, DM, result_path, idxs=None, sensor_eval=None, max_dist=None,
            sensor_eval_super=None, max_occ=None, whitelist_types=['Car', 'Pedestrian', 'Cyclist'],
    ...
    def _results_from_folder(self):
        ...
        part_func = partial(self._run_per_frame_analysis, self.DM, self.result_path,
            self.sensor_eval, self.sensor_eval_super, self.max_dist, self.max_occ, self.whitelist_types)
        res_frame = []
        if self.multiprocess:
            with Pool(8) as p:
                res_frame = list(tqdm(p.imap(part_func, self.idxs_available), total=len(self.idxs_available)))
        else:
    ...
    @staticmethod
    def _run_per_frame_analysis(DM, result_path, whitelist_types, idx):
        raise NotImplementedError

    def _run_per_seq_analysis(self):
        raise NotImplementedError


class ResultManager():
    def __init__(self, idx, detections, truths, truths_dontcare=[], metric='3D_IoU', radius=None, no_white=False, no_black=False):
        """
        """
        self.idx = idx
        self.metric = metric
    ...
    def __str__(self):
        return 'Result Manager\n----{} detections, {} truths\n----{} detections are dontcares\n----{} Assignments, {} FPs, {} FNs'.format(len(self.detections), len(self.truths), len(self.result['det_dontcare']), len(self.result['assigned']), len(self.result['false_positives']), len(self.result['false_negatives']))
    ...
    def run_assignment(self):
        ...
        assert all([tru.origin == self.truths[0].origin for tru in self.truths]), 'All truth origins must be the same for now'

        # Run assignment
        dets = deepcopy(self.detections)
        assignment, A = associate_detections_truths(dets, self.truths_all, self.metric, self.radius)
        self.A = A
        ...
        idx_det_dontcare = [r for r, c in assignment.iterate_over('rows').items() if c in idx_tru_dontcare]
        assigns = {r:c for r, c in assignment.iterate_over('rows').items() if list(c.keys())[0] not in idx_tru_dontcare}
        assignment = OneEdgeBipartiteGraph(assigns, assignment.nrow-len(idx_det_dontcare),
            assignment.ncol-len(idx_tru_dontcare), assignment.cost)
        idx_FN = deepcopy(assignment.unassigned_cols)

        # Make confusion matrix
        self.confusion = np.array([[len(assignment), len(idx_FN)],
                                    [len(idx_FP), 0]])
        ...
        for i in range(len(self.detections)):
            if i in idx_FP:
                self.colors['detections'].append(color_from_object_type('false_positive', self.no_white, self.no_black))
            elif i in idx_det_dontcare:
                self.colors['detections'].append(color_from_object_type('dontcare', self.no_white, self.no_black))
            else:
                self.colors['detections'].append(color_from_object_type('true_positive', self.no_white, self.no_black))
        for i in range(len(self.truths)):
            if i in idx_FN:
                self.colors['truths'].append(color_from_object_type('false_negative', self.no_white, self.no_black))
            elif i in idx_tru_dontcare:
                self.colors['truths'].append(color_from_object_type('dontcare', self.no_white, self.no_black))
            else:
                self.colors['truths'].append(color_from_object_type('truth', self.no_white, self.no_black))
    ...
    def get_objects_of(self, field):
         if field == 'false_positives':
             ...
             return [(self.detections[i], self.truths[list(j.keys())[0]]) for i, j in self.result['assigned'].items()]
         elif field == 'truths':
             return self.truths
         elif field == 'detections':
             return self.detections
...
def associate_detections_truths(detections, truths, metric='3D_IoU', radius=None):
    ...
    assignment = gnn_single_frame_assign(A,
                            algorithm='JVC', all_assigned=False)
    return assignment, A
...
def _get_assignment_cost(det, tru, metric, radius2):
    ...
    if metric == '3D_IoU':
        ...
        if det.T_reference != tru.T_reference:  # convert to det frame
            ...
            center2 = tru.T_reference.coordinates.convert(T_switch @ center2, det.T_reference.coordinates)
        if len(center1) == 2:
        ...
        try:
            ...
            if box_in_fov(tru.box3d, det.box2d.calibration):
                tru_box = tru.box3d.project_to_2d_bbox(det.box2d.calibration, check_origin=True)
            else:
                return np.inf
