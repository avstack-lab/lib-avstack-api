...
from avapi.evaluation.tracking import (
    CLEAR,
    HOTA,
    VACE,
    AvstackTrackDataset,
...
partial_occ = Occlusion.PARTIAL

whitelist_types = {
    "kitti": ["Car", "Cyclist"],
    "nuscenes": ["car", "bicycle", "truck", "bus", "motorcycle"],
...
def init_AV(AVs, i_case, dataset, li_perception, ca_perception, tracking, **kwargs):
    ...
    li_p_in = (
        li_perception[i_case] if isinstance(li_perception, dict) else li_perception
    )
    ca_p_in = (
        ca_perception[i_case] if isinstance(ca_perception, dict) else ca_perception
    )
        ...
        for i, i_trial in enumerate(trials):
            ...
            for i_case in all_cases:
                ...
                AV, frame_results, seq_results = run_case(
                    ...
                    filter_front=filter_front_in,
                    trade_type=trade_in,
                    sensor_eval=sensor_eval_in,
    ...
    **kwargs
    ...
    for frame in tqdm(
        ...
        if max_framerate is None:
            ...
            if (t_curr - t_last) < (1.0 / max_framerate - 1e-5):
                continue
            t_last = t_curr

        # -- run tick
        data_recur = tick_func(
            data_recur,
            AV,
            data_manager,
            SD,
    ...
    t_res = get_agg_tracking_metrics(
        SD,
        AV,
        max_dist=max_dist,
        sensor_eval=sensor_eval,
    ...
    metrics = {
        "perception": p_res[1],
        "tracking": t_res[1],
        "prediction": get_agg_prediction_metrics(
            SD, AV, sensor_eval=sensor_eval, sensor_eval_super=sensor_eval_super
        ),
    }
    if len(c_res) > 0:
    ...
    # -- expand all metrics
    metrics_exp = {
        "Metrics_{}_{}".format(k1, k2): v2
        for k1 in metrics
        for k2, v2 in metrics[k1].items()
    }
...
def tick_standard(
    data_recur, AV, data_manager, SD, frame, filter_front, max_lidar_range, **kwargs
):
    # -- sensor data
    data_manager.push(
        SD.get_lidar(
...
def tick_collaborative(
    ...
    **kwargs
    ...
    for sensor in collaborative_sensors:
        ...
        if (
            curr_time - data_recur[sensor]["t_last_send"]
        ) < 1.0 / collaborative_rate - 1e-4:
            continue  # not ready to make measurement

        # -- convert the raw sensor data to what we want
        if (
            data_recur[sensor]["last_frame_available"]
            == data_recur[sensor]["last_frame_sent"]
        ):
            raise RuntimeError(
                "Asking to send duplicate frames...check data rates {}".format(
                    data_recur[sensor]
                )
            )
        ...
        detections = [
            BoxDetection(sensor, box, obj_type, score)
            for box, obj_type, score in zip(boxes, obj_types, scores)
        ]
        collab_data = DataContainer(
            c_frame, c_timestamp, detections, "collaborative_" + sensor
        )
...
def get_agg_collaborative_metrics(SD, AV, frame_results):
    key_list = [
        "n_collab_detections",
        "n_collab_sensors_in_view",
        "n_collab_sensors_total",
    ]
    c_res = {}
    k_agg = "{}_median"
...
def get_agg_percep_metrics(
    SD,
    AV,
    sensor_eval,
    sensor_eval_super="main_lidar",
    max_dist=80,
    max_occ=partial_occ,
    multiprocess=False,
):
    ...
    res = {
        k: get_percep_results_from_folder(
            SD,
            result_path=v.save_folder,
            sensor_eval=sensor_eval,
            sensor_eval_super=sensor_eval_super,
            max_dist=max_dist,
            max_occ=max_occ,
        ...
        for k, v in AV.perception.items()
    }
    ...
    per_seq_res = {
        "{}_{}".format(k1, k2): v2
        for k1 in per_seq_res
        for k2, v2 in per_seq_res[k1].items()
    }
    return per_frame_res, per_seq_res


def get_agg_tracking_metrics(
    SD,
    AV,
    sensor_eval,
    sensor_eval_super="main_lidar",
    trk_name="box-tracker",
    objects="all_objects",
    max_dist=80,
    max_occ=partial_occ,
    multiprocess=False,
...
def get_agg_prediction_metrics(
